...
原因：未知，但是本人没有遇到过这个问题，也不太理解为什么会出现这个问题，所有assert（断言）均为闭环调用，再您没更改过我的代码的情况下应该不会出现。
...
写在最后：
如果您看到这里依然没有解决您的问题，并且您认为并不是您配置文件或其他相关配置出错，欢迎给我发邮件（zhaoyu@sniper97.cn）或提issues。

如果您给我发邮件，十分建议您在邮件正文添加一个大写字母A以作为您阅读过本页的标记，否则可能不会回复。
